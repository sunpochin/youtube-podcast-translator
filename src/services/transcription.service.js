...
async function downloadYouTubeAudio(videoId) {
  const fileBase = path.join(os.tmpdir(), `yt-audio-${videoId}-${crypto.randomUUID()}`);
  const outputTemplate = `${fileBase}.%(ext)s`;
  ...
  const matches = await fs.readdir(os.tmpdir());
  const fileName = matches.find((name) => name.startsWith(path.basename(fileBase)));
  if (!fileName) {
    throw new Error('yt-dlp 下載完成但找不到輸出音訊檔');
  }
